  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        >
          <h2 id="cat-list-heading" className="text-base font-semibold text-slate-900 dark:text-slate-100">
            Tus categorías
          </h2>
          <ul className="mt-4 flex flex-col gap-2">
            {categories.map((c) => (
              <li
                key={c.id}
                className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-3 dark:border-slate-800 dark:bg-slate-950/50"
              >
                <span
                  className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-900"
                  style={{ color: c.color }}
                >
